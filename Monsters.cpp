    ...
    for(unsigned long long i = 0; i <members.size(); i++){
        if(Globals::DISTANCE(members[i].GetPosition(),render_center) < render_distance){
            ...
            for(unsigned long long j = 0; j <members.size(); j++){
                if(i != j && Globals::DISTANCE(members[j].GetPosition(),render_center) < render_distance && members[i].GetGlobalBounds().intersects(members[j].GetGlobalBounds()) && robot_hit_sound_cooldown.getElapsedTime().asSeconds()>.2){
...
void Monsters::Interact(Player &player){
    if(!player.IsFalling()){
        for(auto &it:members){
            ...
            if(!it.GetMovable() && std::abs(Globals::DISTANCE(it.GetPosition(),player.GetPosition()))< 350){
...
void Monsters::CheckIfDead(){
    for(unsigned long long i =0; i<members.size();i++){
        if(!members[i].GetAlive()){
            if(members[i].GetMovable())
                robot_death.play();
            else if(!members[i].GetMovable())
                boom_death.play();
